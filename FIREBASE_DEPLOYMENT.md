...
## Step 4: Cloud Functions (Admin API)

The repository now includes a lightweight Firebase Function that returns the list of registered users stored in Firestore (`users` collection). This is deployed as `listUsers` and is protected by a simple API key header.

### 4.1 Add Admin API Key
Add a GitHub secret named `ADMIN_API_KEY` with a strong random value (e.g. generated by `openssl rand -hex 32`). Also add `ADMIN_API_KEY` to your Firebase Functions environment when deploying manually or via CI.

### 4.2 How the function is protected
The function expects an HTTP header `x-admin-key` equal to the `ADMIN_API_KEY` value. Requests without the correct key receive `403 Forbidden`.

### 4.3 Example call (local or server)
```bash
curl -H "x-admin-key: $ADMIN_API_KEY" https://us-central1-<PROJECT_ID>.cloudfunctions.net/listUsers
```

### 4.4 CI/CD
The GitHub Actions workflow was updated to deploy functions alongside hosting. Ensure your `FIREBASE_SERVICE_ACCOUNT_KEY` secret is present and contains the service account JSON used for deployment.

## Step 5: Image Optimization (Optional)
